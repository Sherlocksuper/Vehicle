  ...
  return (
    <div>
      ...
      >
        <Input
          prefix="测试配置名称："
          value={testProcessN?.testName}
          disabled={modalType === "show"}
          onChange={(e) => {
            setTestProcessN({
              ...testProcessN,
        ...
        <Select
          ...
          onSelect={(value) => {
            ...
            newTestProcess!.template = testTemplateList.find(
              (item) => item.id === value
            ) as ITemplate;
          ...
          columns={[
            {
              ...
              render: (_, __, index) => (
                <Select
                  onSelect={(value) => {
                    const newTestProcess = testProcessN;
                    newTestProcess!.testObjectNs[index].vehicle =
                      vehicleList.find((item) => item.id === value) as IVehicle;
                    setTestProcessN(newTestProcess);
                  }}
                  size={"middle"}
                  style={{ width: 200 }}
              ...
              render: (_, __, index) => (
                <Select
                  ...
                  onChange={(value) => {
                    console.log(value);
                    const newTestProcess = testProcessN;
                    newTestProcess!.testObjectNs[index].project = value.map(
                      (item: number) =>
                        projectList.find(
                          (project) => project.id === Number(item)
                        ) as IProject
                    );
                    setTestProcessN(newTestProcess);
                  }}
                ...
                <span>
                  <a
                    onClick={() => {
                      setTestProcessN({
                        ...testProcessN,
                        testObjectNs: testProcessN?.testObjectNs?.filter(
                          (item) => item !== record
                        ),
                      } as ITestProcessN);
                    }}
